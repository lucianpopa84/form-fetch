...
2. without the \<form> element (or keep the \<form> element and use `preventDefault()` on form submit event listner), get the input fields values and send them using the fetch API https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch or XMLHttpRequest https://javascript.info/xmlhttprequest
